...
def make_train(config: Dict[str, Any]):
    ...
    def train(rng: jnp.ndarray):
        ...
        def _update_step(update_runner_state, _):
            ...
            def _log_callback(payload, env_step):
                if LOGGER is not None:
                    payload_py = jax.tree.map(lambda x: float(x), payload)
                    LOGGER.log(payload_py, step=int(env_step))
            ...
            if config["use_wandb"]:
                ...
                returns = (metric["returned_episode_returns"][:, :, 0] * mask_f).sum() / denom
                ...
                jax.experimental.io_callback(_log_callback, None, payload, env_step)
