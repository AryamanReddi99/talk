...
def make_train(config: Dict[str, Any]):
    ...
    def train(rng: jnp.ndarray):
        ...
        def _update_step(update_runner_state, _):
            ...
            mask_f = episode_mask.astype(jnp.float32)
            denom = jnp.maximum(mask_f.sum(), 1.0)
            return_avg = (
                metric["returned_episode_returns"][:, :, 0] * mask_f
            ).sum() / denom
            episode_length_avg = (
                metric["returned_episode_lengths"][:, :, 0] * mask_f
            ).sum() / denom
            win_rate = (metric["returned_won_episode"][:, :, 0] * mask_f).sum() / denom
            ...
            log_metric = {
                ...
                "num_episodes_completed": episode_mask.sum(),
            ...
            def _log_callback(metric_dict):
                if LOGGER is not None:
                    payload_py = jax.tree.map(lambda x: float(x), metric_dict)
                    log_step = int(payload_py["update_step"])
                    LOGGER.log(payload_py, step=log_step)

            if config["use_wandb"]:
                jax.experimental.io_callback(_log_callback, None, log_metric)
            ...
            return (runner_state, update_step), log_metric
