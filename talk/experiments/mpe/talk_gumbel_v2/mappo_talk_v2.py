...
def make_train(config: Dict[str, Any]):
    ...
    def _talk_env_step(
        ...
        new_h, sig, cb, onehot, logits, _aux, _comm, _diag = jax.vmap(_one_env)(
            ...
            def _update_epoch(update_state, _):
                def _update_minbatch(train_states, batch_info):
                    ...
                    def _actor_loss(params, init_h, ps0, pc0, po0, mb_traj, mb_gae):
                        pi, aux_loss, diag = _actor_trajectory(
                            actor_network.apply,
                            params,
                            init_h,
                            ps0,
                        ...
                        actor_loss = loss_actor - config["ent_coef"] * entropy + aux_coef * aux_loss
                        return actor_loss, (
                            loss_actor,
                            entropy,
                            ratio,
...
@hydra.main(version_base=None, config_path=".", config_name="config_mappo_talk_v2")
def main(config):
    global LOGGER
    try:
        ...
        print("Compiling MAPPO-Talk-V2 MPE...")
