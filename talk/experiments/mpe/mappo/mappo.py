"""Team-shared feedforward MAPPO on JaxMARL MPE (homogeneous single team)."""
...
def make_train(config: Dict[str, Any]):
    ...
    def train(rng: jnp.ndarray, exp_id: int):
        ...
        actor_params = actor_network.init(
            rng_actor, jnp.zeros((num_actors, obs_dim), dtype=jnp.float32)
        )
            ...
            def _update_epoch(update_state, _):
                ...
                def _update_minibatch(train_states, minibatch):
                    ...
                    def _actor_loss(params):
                        pi = actor_network.apply(params, minibatch.obs)
                    ...
                    def _critic_loss(params):
                        ...
                        clipped = minibatch.old_value + (value - minibatch.old_value).clip(
                            -clip_eps, clip_eps
                        )
