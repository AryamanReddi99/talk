"""Team-shared MAPPO + Mordatch-style discrete communication on JaxMARL MPE.

Faithful Mordatch channel on a MAPPO-GRU backbone: agents emit soft
Gumbel-softmax symbols; receivers process every sender's previous message with a
shared-weight GRU (per-sender memory + sender id), max-pool over senders, and a
grounding aux loss predicts each sender's masked private obs. A marginal-entropy
sparsity penalty optionally compacts the vocabulary. Train return uses soft
messages; the test-return rollout uses hard (argmax) messages to measure the
soft->hard deployment gap.
"""
...
from talk.environments.mpe.rollout_viz import log_mordatch_rollout_video_callback
...
def make_train(config: Dict[str, Any]):
    ...
    msg_hidden_size = int(config["msg_hidden_size"])
    gumbel_tau = float(config["gumbel_tau"])
    obs_pred_coef = float(config["obs_pred_coef"])
    word_coef = float(config["word_coef"])
    ...
    def rollout_io_callback(should_log, exp_id, update_step, log_step, actor_params):
        log_mordatch_rollout_video_callback(
            should_log,
            exp_id,
            update_step,
            actor_params,
    ...
    def _mordatch_env_step(
        ...
        def _one_env(h_e, obs_e, done_e, pm_e, pmem_e, key_e, pos_e):
        ...
        new_h, new_msg, new_msg_mem, logits, _opl, _msp, _diag = jax.vmap(_one_env)(
            h, obs, done, prev_msg, prev_msg_mem, msg_keys, ally_pos
        )
        return (
    ...
    def _actor_trajectory(
        ...
        params,
        init_h: jnp.ndarray,
        init_prev_msg: jnp.ndarray,
        init_prev_msg_mem: jnp.ndarray,
        traj,
    ):
        def _scan_step(carry, inputs):
            ...
            new_h, new_msg, new_msg_mem, logits, opl, msp, diag = jax.vmap(_one_env)(
                h, obs_t, done_t, pm, pmem, msg_key_t, ally_pos_t
            )
            ep_done = global_done_t[:, 0:1, None]
            new_msg = jnp.where(ep_done, 0.0, new_msg)
            new_msg_mem = jnp.where(global_done_t[:, 0:1, None, None], 0.0, new_msg_mem)
            step_out = (logits, opl, msp, diag)
            return (new_h, new_msg, new_msg_mem), step_out

        _, (logits, opl, msp, diag) = jax.lax.scan(
            _scan_step,
            (init_h, init_prev_msg, init_prev_msg_mem),
            (traj.obs, traj.done, traj.ally_positions, traj.global_done, traj.msg_key),
            unroll=scan_unroll,
        )
        obs_pred_loss = jnp.mean(opl)
        marginal = jnp.mean(msp, axis=(0, 1))  # mean over steps, envs -> (V,)
        marginal = marginal / (jnp.sum(marginal) + 1e-8)
        marginal_entropy = -jnp.sum(marginal * jnp.log(marginal + 1e-8))
    ...
    def train(rng: jnp.ndarray, exp_id: int):
        ...
        actor_network = ActorMordatchRNN(
        ...
        actor_params = actor_network.init(
            ...
            jnp.zeros((num_agents, vocab_size)),
            jnp.zeros((num_agents, num_agents, msg_hidden_size)),
        ...
        def _collect_eval(actor_params, rng):
            """Deployment rollout with hard (argmax) messages. Returns mean
            ...
            prev_msg = jnp.zeros((num_envs, num_agents, vocab_size))
            prev_msg_mem = jnp.zeros((num_envs, num_agents, num_agents, msg_hidden_size))
            last_done = jnp.zeros((num_actors,), dtype=bool)

            def _eval_step(carry, _):
                ac_h, env_state, last_obs, last_done, pm, pmem, rng = carry
                ...
                ac_h, new_msg, new_msg_mem, logits = _mordatch_env_step(
                    actor_network.apply,
                    actor_params,
                    ac_h,
                    obs_batch,
                ...
                pm_next = jnp.where(global_done[:, None, None], 0.0, new_msg)
                pmem_next = jnp.where(global_done[:, None, None, None], 0.0, new_msg_mem)
                carry = (ac_h, env_state, obsv, done_batch, pm_next, pmem_next, rng)
                out = (info["returned_episode"], info["returned_episode_returns"])
                return carry, out

            init = (ac_h, env_state, obsv, last_done, prev_msg, prev_msg_mem, rng)
        ...
        def _update_step(update_runner_state, _):
            ...
            def _env_step(runner_state, _):
                ...
                done_batch = batchify(done, env.agents, num_actors).squeeze()
                global_done = done["__all__"]
                prev_msg_next = jnp.where(global_done[:, None, None], 0.0, new_msg)
                prev_msg_mem_next = jnp.where(
                    global_done[:, None, None, None], 0.0, new_msg_mem
                )
            ...
            prev_msg_init = runner_state[5]
            prev_msg_mem_init = runner_state[6]
            ...
            def _update_epoch(update_state, _):
                def _update_minbatch(train_states, batch_info):
                    ...
                    def _actor_loss(params, init_h, pm0, pmem0, mb_traj, mb_gae):
                        pi, obs_pred_loss, marginal_entropy, diag = _actor_trajectory(
                            actor_network.apply,
                            params,
                            init_h,
                            pm0,
                    ...
                    actor_out, actor_grads = actor_grad_fn(
                        actor_state.params, ac_h, pm_init, pmem_init, traj_mb, gae_mb
                    )
                    ...
                    loss_info = {
                        ...
                        "obs_pred_loss": actor_out[1][5],
                        "marginal_entropy": actor_out[1][6],
                ...
                prev_msg_env = prev_msg_init
                prev_msg_mem_env = prev_msg_mem_init
                traj_env = Transition(
                ...
                prev_msg_mb = prev_msg_env[perm].reshape(
                    num_minibatches, mb_envs, num_agents, vocab_size
                )
                prev_msg_mem_mb = prev_msg_mem_env[perm].reshape(
                    num_minibatches, mb_envs, num_agents, num_agents, msg_hidden_size
                )
                traj_mb = Transition(
...
@hydra.main(version_base=None, config_path=".", config_name="config_mappo_mordatch")
def main(config):
    global LOGGER
    try:
        ...
        print("Compiling MAPPO-Mordatch MPE...")
