"""Independent PPO (IPPO) on the MessageBox multi-agent environment."""
    ...
    def train(rng: PRNGKeyArray, exp_id: int):
        def train_setup(rng: PRNGKeyArray):
            ...
            def init_agent(agent_idx: int, rng_a: PRNGKeyArray):
                ...
                actor_ts = TrainState.create(
                    ...
                    params=actor.init(rng_actor, init_obs),
                    tx=tx_actor,
                )
                critic_ts = TrainState.create(
                    apply_fn=critic.apply,
                    params=critic.init(rng_critic, init_obs),
                    tx=tx_critic,
                )
            ...
            def _update_epoch(update_state: UpdateState, _):
                ...
                def _update_minibatch(carry, minibatch):
                    ...
                    def _critic_loss(params, traj_mb, tgt_mb):
                        value = c_ts.apply_fn(params, traj_mb.obs)
        ...
        def _train_loop(runner_state: RunnerState, _):
            ...
            def _env_step(runner_state: RunnerState, _):
                ...
                def act_one_env(obs_e, key_e, actor_ts, critic_ts):
                    pi = actor_ts.apply_fn(actor_ts.params, obs_e)
                    value = critic_ts.apply_fn(critic_ts.params, obs_e)
                ...
                values_list = []
                for i in range(num_agents):
                    a, lp, v = jax.vmap(act_one_env, in_axes=(0, 0, None, None))(
                        obs_a[:, i], action_keys[i], actor_states[i], critic_states[i]
                    )
                ...
                def step_one_env(key, state, actions_per_agent):
                    new_obs, new_state, rewards, done, _ = env.step(key, state, actions_per_agent)
                    return new_obs, new_state, rewards, done
            ...
            for i in range(num_agents):
                ...
                last_values.append(c_ts.apply_fn(c_ts.params, runner_state.obs[:, i]))
...
@hydra.main(version_base=None, config_path=".", config_name="config_ippo")
def main(config):
    global LOGGER
    try:
        ...
        LOGGER = WandbMultiLogger(
            ...
            job_type=config["algorithm"] + f"{config['custom_name']}",
        ...
        print("Compiling IPPO...")
        train_fn = jax.jit(jax.vmap(make_train(config)))
        print("Running...")
        out = jax.block_until_ready(train_fn(rng_seeds, exp_ids))
    finally:
        LOGGER.finish()
        print("Finished.")
