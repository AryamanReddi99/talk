"""MAPPO on MessageBox with ground-truth actor observations for all agents."""
...
def _actor_obs(obs: FloatArray, sender_idx: int) -> FloatArray:
    """Broadcast the sender's ground-truth one-hot message to every agent."""
    ground_truth_obs = obs[..., sender_idx, :]
    return jnp.broadcast_to(ground_truth_obs[..., None, :], obs.shape)
    ...
    def train(rng: PRNGKeyArray, exp_id: int):
        ...
        def _train_loop(runner_state: RunnerState, _):
            ...
            def _env_step(runner_state: RunnerState, _):
                ...
                actor_obs = _actor_obs(obs_a, sender_idx)
                global_state = _global_state(actor_obs)
            ...
            global_state_last = _global_state(_actor_obs(runner_state.obs, sender_idx))
            last_values = []
            for i in range(num_agents):
...
@hydra.main(version_base=None, config_path=".", config_name="config_mappo_ground_truth")
def main(config):
    global LOGGER
    try:
        ...
        print("Compiling MAPPO (ground-truth actor obs)...")
