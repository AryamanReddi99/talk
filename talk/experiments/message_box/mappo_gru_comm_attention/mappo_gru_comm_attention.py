"""MAPPO with shared GRU actors/critics and neighbor attention on MessageBox."""
...
from talk.networks.gru import ActorDiscreteCommAttnRNN, CriticDiscreteRNN, ScannedRNN
...
def _encode_step(
    ...
    """Encode all agents in one shared forward pass. obs: (E, A, D)."""
    num_envs = obs.shape[0]
    obs_ae = obs.transpose(1, 0, 2)
    batch = num_agents * num_envs
    obs_scan = obs_ae.reshape(1, batch, obs_ae.shape[-1])
    done_scan = jnp.broadcast_to(done[jnp.newaxis, :], (num_agents, num_envs)).reshape(
        1, batch
    )
    new_hs, msgs = actor_apply(
        params,
        actor_hs.reshape(batch, -1),
        obs_scan,
        done_scan,
        method=ActorDiscreteCommAttnRNN.encode,
    )
    msg_dim = msgs.shape[-1]
    return (
        new_hs.reshape(num_agents, num_envs, -1),
        msgs.reshape(num_agents, num_envs, msg_dim),
    )
...
def _encode_trajectory(
    ...
    """Return messages with shape (T, A, E, msg_dim)."""

    def step(carry, inputs):
        hs, obs_t, done_t = carry, inputs[0], inputs[1]
        new_hs, msgs = _encode_step(
            actor_apply, params, hs, obs_t, done_t, num_agents
        )
        return new_hs, msgs

    _, msgs = jax.lax.scan(step, init_hs, (obs, done))
    return msgs
...
def _act_batched(
    ...
    """own_msgs: (N, D), neighbor_msgs: (N, S, D), neighbor_mask: (N, S)."""
    return actor_apply(
        params,
        own_msgs,
        neighbor_msgs,
        neighbor_mask,
        method=ActorDiscreteCommAttnRNN.act,
    )
...
def make_train(config: dict):
    ...
    def train(rng: PRNGKeyArray, exp_id: int):
        def train_setup(rng: PRNGKeyArray):
            ...
            actor = ActorDiscreteCommAttnRNN(
                ...
                def _update_minibatch(carry, minibatch):
                    ...
                    def _actor_loss(params, init_h, traj_mb, adv_mb):
                        ...
                        neighbor_msgs, neighbor_mask = _gather_neighbor_slots(
                            msgs, num_agents, agent_axis=1
                        )
                        own_flat = msgs.transpose(0, 2, 1, 3).reshape(-1, msg_dim)
                        nb_flat = neighbor_msgs.transpose(2, 3, 0, 1, 4).reshape(
                            -1, MAX_NEIGHBOR_SLOTS, msg_dim
                        )
                        mask_flat = neighbor_mask.transpose(2, 3, 0, 1).reshape(
                            -1, MAX_NEIGHBOR_SLOTS
                        )
                        pi = _act_batched(
                            a_ts.apply_fn, params, own_flat, nb_flat, mask_flat
                        )
                        log_prob = pi.log_prob(traj_mb.action.reshape(-1)).reshape(
                            traj_mb.action.shape
                        )
                        ...
                        entropy = (pi.entropy().reshape(traj_mb.action.shape) * mask).sum()
        ...
        def _train_loop(runner_state: RunnerState, _):
            ...
            def _env_step(runner_state: RunnerState, _):
                ...
                new_actor_hs, all_msgs = _encode_step(
                    actor_ts.apply_fn,
                    actor_ts.params,
                    actor_hs,
                    obs_a,
...
@hydra.main(
    version_base=None,
    config_path=".",
    config_name="config_mappo_gru_comm_attention",
)
def main(config):
    global LOGGER
    try:
        ...
        print("Compiling MAPPO with shared GRU + attention...")
