"""MAPPO with shared pre-attention GRU actors/critics and state-in policy head.

Actor flow: concat(obs, h) -> message -> attention -> concat(msg, context)
-> GRU -> state decoder -> concat(gru_out, stop_grad(state_pred)) -> action.
State decoder is trained only via MSE against global_state.
"""
...
from talk.networks.gru import (
    ActorDiscretePreAttnCommStateInRNN,
    CriticDiscreteRNN,
    ScannedRNN,
)
...
def _message_step(
    ...
    msgs = actor_apply(
        ...
        method=ActorDiscretePreAttnCommStateInRNN.message,
    )
...
def _actor_trajectory(
    ...
    """Run message -> attention -> GRU -> action over time.

    Returns logits (T, E, A, A_dim) and state preds (T, E, A, G).
    """
    ...
    def step(h, inputs):
        ...
        state_pred = state_pred.reshape(num_agents, obs_t.shape[0], -1)
        return new_h.reshape(num_agents, obs_t.shape[0], -1), (logits, state_pred)

    _, (logits, state_preds) = jax.lax.scan(step, init_hs, (obs, done))
    return logits.transpose(0, 2, 1, 3), state_preds.transpose(0, 2, 1, 3)
...
def _act_batched(
    ...
    return actor_apply(
        ...
        neighbor_mask,
        dones,
        method=ActorDiscretePreAttnCommStateInRNN.act,
    )
...
def make_train(config: dict):
    ...
    def train(rng: PRNGKeyArray, exp_id: int):
        def train_setup(rng: PRNGKeyArray):
            ...
            actor = ActorDiscretePreAttnCommStateInRNN(
            ...
            def _update_epoch(update_state: UpdateState, _):
                ...
                def _update_minibatch(carry, minibatch):
                    ...
                    def _actor_loss(params, init_h, traj_mb, adv_mb):
                        ...
                        state_mse = jnp.square(
                            state_preds - state_target[:, :, None, :]
                        ).mean()
                        loss = (
                            loss_actor
...
@hydra.main(
    version_base=None,
    config_path=".",
    config_name="config_mappo_att_grucomm_dub_state_in",
)
def main(config):
    global LOGGER
    try:
